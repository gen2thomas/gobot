...
// HMC5883LDriver is a Driver for a HMC5883 3 axis digital compass
...
//
func NewHMC5883LDriver(a Connector, options ...func(Config)) *HMC5883LDriver {
	hmc := &HMC5883LDriver{
		name:            gobot.DefaultName("HMC5883L"),
		connector:       a,
		samplesAvg:      8,
		outputRate:      15000,
		applyBias:       0,
...
func (h *HMC5883LDriver) Name() string { return h.name }

// SetName sets the name for this Driver
func (h *HMC5883LDriver) SetName(n string) { h.name = n }

// Connection returns the connection for this Driver
func (h *HMC5883LDriver) Connection() gobot.Connection { return h.connector.(gobot.Connection) }

// Start initializes the HMC5883L
func (h *HMC5883LDriver) Start() (err error) {
	bus := h.GetBusOrDefault(h.connector.GetDefaultBus())
	address := h.GetAddressOrDefault(hmc5883lDefaultAddress)
...
func (h *HMC5883LDriver) Halt() (err error) { return }
...
func (h *HMC5883LDriver) ReadRawData() (x int16, y int16, z int16, err error) {
	// set to the initial register
	err = h.connection.WriteByte(hmc5883lAxisX)

	// use continuous read to get the data
	data := make([]byte, 6)
	cnt, err := h.connection.Read(data)
	if err != nil {
		return
	}
	if cnt != 6 {
		err = fmt.Errorf("read %d values, 6 values expected", cnt)
		return
	}
